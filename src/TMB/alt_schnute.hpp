...
//
template<class Type>
Type alt_schnute(objective_function<Type>* obj) {
  ...
  Type f;
  tp  = indices_class;
  vector <Type> logB0(no_ind);
  vector <Type> B0_ctrl(no_ind);

  //Type B0;

  //f = f_calc[0];
  catch_pred[0] = obs_catch[0];//f * biomass[0] * (1.0 - exp(-(f+m))) / (f+m);
  mu[0] = 0.5;//(f*(1 - exp(-m)) - ((m*exp(-m))*(1-exp(-f))))/ (f*(1 - exp(-m))*(1-exp(-m-f)));
  //theta = indices_ts[0];
  //q = qhat[0];
  //B0 = (obs_ind(0,0) + q*((theta*(1 - mu[0]*(1-sigma)))*catch_pred[0]))/q*(1-theta*(1-sigma));
  for(int j=0; j<no_ind; j++){
    ...
    q = qhat[j];
    if(obs_ind(j,0)>0){
      logB0[j] = log((obs_ind(j,0) + q*((theta*(1 - mu[0]*(1-sigma)))*obs_catch[0]))/q*(1-theta*(1-sigma)));
      B0_ctrl[j]= 1;
    }
  }

  biomass[0] = exp(logB0.sum()/ B0_ctrl.sum());
