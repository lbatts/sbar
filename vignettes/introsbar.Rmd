...
plot(y=rec_mw,x=years,type="b",ylim=c(0,1.5),ylab="mean wight (kg)")
lines(y=pe_mw,x=years,col=2,type="b")
lines(y=mw,x=years,col=3,type="b")
legend("topright", legend = c("rec_mw (recruit mean wt)", "pe_mw (previously exploited biomass mean wt)", "mw (Total biomass mean wt)"), col = 1:3, lty = 1,cex=0.5)
```
...
Next we need growth parameters. This aspect is discussed in more detail in the schnute vignette but for now just note that we fit a linear model to overall mean weights and previously exploited mean weights ($pe\_mw_{t+1}$ vs $mw_t$)
...
```{r assessment sorig}
obj<-schnute_orig(catch_b = catch_kg, indices_b = obs, ts = timing, mwts = mwts, rho = rho1, W = W1, start_sigma = sigma_st)
fit <- fit_tmb(obj = obj, getHessian = T ,quiet=T,control = list(trace=0))
...
x <- makesbarclass(obs.srep,survnames,catch_kg,obs,years)
...
```{r assessment sobs}
obj<-schnute_obserror(catch_b = catch_kg, indices_b = obs, ts = timing, mwts = mwts, rho = rho1, W = W1, start_sigma = sigma_st)
fit <- fit_tmb(obj = obj, getHessian = T ,quiet=T,control = list(trace=0))
obs.srep<-summary(fit$SD)
```
